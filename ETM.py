...
if version[0] < 3:
    print("[X] ETM ne peut fonctionner correctement que sous python 3.0.0 minimum et votre version est actuellement la {}.{}.{} ; Veuillez mettre à jour python et réessayer.".format(
        ...
        def run(self, connection: socket, server_error_code: int, client_ip: str):
            if server_error_code == 1:
                try:
                    ...
                    RSA_keys.append(PKCS1_OAEP.new(
                        RSA.import_key(RSA_keys[3])))
                except ConnectionResetError:
                    pass
                modify_cache("reset_cache")
                listener_process = Process(target=listener, args=(
                    connection, self.client_ip, RSA_keys[0].export_key()))
                listener_process.start()
                print(
                    "\n[+] Nouveau client connecté ({}:25115) ! Dès que vous voudrez quitter la discussion, entrez Exit.\n".format(client_ip))
                while True:
                    message = input("\n=> ")
                    modify_cache("save_message", "[{}] {}".format(str(self.ip), str(
                        datetime.now().strftime('%d-%m-%Y %H:%M:%S'))), message)
                    try:
                        connection.send(RSA_keys[4].encrypt(
                            bytes(message, "utf-8")))
                    except ConnectionResetError:
                        pass
                    ...
                    if modify_cache("get_listening_status") == False:
                        ...
                        user_choice = verify_user_entry("str", user_choice, [
                                                        "o", "n"], "[?] Voulez-vous enregistrer cette conversation ? [O/N]")
    ...
    def modify_cache(request: str, key: str = "", value: str = ""):
        ...
        if request == "reset_cache":
            ...
            if not path.exists("./saved_conversation.txt"):
                with open("./saved_conversation.txt", "x") as saved_conversation_file:
                    saved_conversation_file.close()
            else:
                with open("./saved_conversation.txt", "w") as saved_conversation_file:
                    saved_conversation_file.close()
    ...
    def listener(s: socket, pair_ip: str, key: RSA.RsaKey):
        ...
        while True:
            ...
            if decrypted_data.lower() == "exit":
                ...
                if modify_cache("get_listening_status") == True:
                    print("\n=> [{}] {}\n\t{}".format(str(pair_ip), str(
                        datetime.now().strftime('%d-%m-%Y %H:%M:%S')), decrypted_data))
                try:
                    modify_cache("save_message", "[{}] {}".format(str(pair_ip), str(
                        datetime.now().strftime('%d-%m-%Y %H:%M:%S'))), decrypted_data)
                except:
                    pass
        ...
        try:
            ...
            if server_error_code == ("1"):
                ...
                listener_process = Process(
                    target=listener, args=(s, ip, RSA_keys[0].export_key()))
                listener_process.start()
                while True:
                    message = input("\n=> ")
                    modify_cache("save_message", "[{}] {} ".format(str(ip), str(
                        datetime.now().strftime("%d-%m-%Y %H:%M:%S"))), message)
                    try:
    ...
    def regenerate_user_config(force_regeneration: bool):
        if path.exists("./user.json") and force_regeneration == False:
            with open("./user.json", "r") as user_config_file:
                ...
                try:
                    user_config_content["name"], user_config_content[
                        "description"], user_config_content["ip"], user_config_content["status"]
                except KeyError:
                    print(colored(
            ...
            if path.exists("./user.json"):
                ...
                with open("./user.json", "x") as user_config_file:
                    user_config_file.close()
    ...
    def modify_user_status(user_choice: int = -1):
        if user_choice == -1:
            ...
            user_choice = verify_user_entry("int", user_choice, [
                                            1, 2], "[?] Voulez-vous être en ligne ou hors-ligne ? [1/2]")
        if user_choice == 1:
    ...
    def add_contact(name: str, description: str, ip: str):
        if not path.exists("./contacts.csv"):
            with open("./contacts.csv", "x") as contacts_file:
                contacts_file.close()
        with open("./contacts.csv", "a") as contacts_file:
            contacts_file_writer = writer(
                contacts_file, delimiter=" ", quotechar="\"", quoting=QUOTE_MINIMAL)
            contacts_file_writer.writerow(
                [str(name), str(description), str(ip)])
            contacts_file.close()
    ...
    def display_contacts():
        ...
        with open("./contacts.csv", "r") as contacts_file:
            contacts_file_reader = reader(
                contacts_file, delimiter=" ", quotechar="\"")
            i = 1
            for row in contacts_file_reader:
                if len(row) > 0:
                    sleep(0.1)
                    print("\n[-] Contact numéro {}:\n\tNom: {}\n\tDescription: {}\n\tIP: {}\n".format(
                        str(i), str(row[0]), str(row[1]), str(row[2])))
                    i = i + 1
                else:
                    print("\n")
            contacts_file.close()
    ...
    ╚══════╝   ╚═╝   ╚═╝     ╚═╝
        ...
        while True:
            ...
            user_choice = verify_user_entry("int", user_choice, [
                                            1, 2, 3, 4, 5, 6, 7, 8, 9], "[?] Quel Est Votre Souhait ?\n\t1/ Héberger un salon ;\n\t2/ Me connecter à un salon ;\n\t3/ Modifier mes réglages ;\n\t4/ Afficher mes contacts ;\n\t5/ Ajouter un contact ;\n\t6/ Modifier mon statut ;\n\t7/ Afficher l'aide ;\n\t8/ C'est quoi ETM ? ;\n\t9/ Quitter ETM ;\n")
            ...
            if user_choice == 1:
                ...
                user_choice = verify_user_entry("int", user_choice, [
                                                1, 2, 3], "[?] Voulez-vous utiliser l'adresse IP enregistrée, votre adresse IP locale (automatiquement détectée) ou entrer une adresse vous-même ? [1/2/3]")
                if user_choice == 1:
                    ...
                    ip = verify_user_entry(
                        "str", ip, [], "[?] Veuillez entrer l'adresse IP que vous souhaitez utiliser.")
                    start_server(ip)
                ...
                user_choice = verify_user_entry("int", user_choice, [
                                                1, 2], "[?] Voulez-vous vous connecter à un contact enregistré ou non enregistré ? [1/2]")
                if user_choice == 1:
                    name = input(
                        "\n[?] Quel est le nom de votre contact ?\n=> ")
                    name = verify_user_entry(
                        "str", name, [], "[?] Quel est le nom de votre contact ?")
                    ...
                    ip = verify_user_entry(
                        "str", ip, [], "[?] Quelle est l'adresse IP de votre contact ?")
                    start_client(ip)

            elif user_choice == 3:
                key = input(
                    "\n[?] Quelle clé voulez-vous modifier ? [description / name / ip]\n=> ")
                key = verify_user_entry("str", key, [
                                        "description", "name", "ip"], "[?] Quelle clé voulez-vous modifier ? [description / name / ip]")
                value = input(
                    "\n[?] Quelle nouvelle valeur voulez-vous attribuer à la clé {} ?\n=> ".format(key.lower()))
                value = verify_user_entry("str", value, [], str(
                    "[?] Quelle nouvelle valeur voulez-vous attribuer à la clé {} ?".format(key.lower())))
                ...
                contact_name = input(
                    "\n[?] Quel est le nom de votre contact ?\n=> ")
                contact_name = verify_user_entry(
                    "str", contact_name, [], "[?] Quel est le nom de votre contact ?")
                contact_description = input(
                    "\n[?] Quelle est la desription de votre contact ?\n=> ")
                contact_description = verify_user_entry(
                    "str", contact_description, [], "[?] Quelle est la desription de votre contact ?")
                contact_ip = input(
                    "\n[?] Quelle est l'adresse ip de votre contact ?\n=> ")
                contact_ip = verify_user_entry(
                    "str", contact_ip, [], "[?] Quelle est l'adresse ip de votre contact ?")
                ...
                user_choice = verify_user_entry("int", user_choice, [
                                                1, 2], "[?] Voulez-vous passer en mode en ligne ou en mode hors-ligne ? [1/2]")
